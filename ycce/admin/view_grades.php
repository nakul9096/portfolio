...
if ($selected_test) {
    // Fetch test details for the header
...
<style>
    ...
    .btn-back-sharp:hover { color: #0f172a; }
...
<div class="report-body min-h-screen bg-[#f8fafc] py-10 px-6 antialiased">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8 no-print">
            <a href="dashboard.php" class="btn-back-sharp inline-flex items-center gap-2">
        ...
        <div id="print-header" class="text-center">
            ...
            <div class="mt-4 flex justify-between items-end italic text-xs font-bold">
                ...
                <span>Max Marks: <?= $test_details['total_marks'] ?? '' ?></span>
            </div>
        </div>
        ...
        <?php else: ?>
            ...
            <div class="flex justify-between items-center mb-6 no-print">
                ...
                <button onclick="exportTableToCSV('YCCE_Report_<?= $selected_test ?>.csv')" class="text-[10px] font-black uppercase tracking-widest text-indigo-600 hover:underline">
                        ...
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y-2 divide-slate-900">
                                ...
                                <tr class="hover:bg-indigo-50/50 transition-colors">
                                    <td class="px-6 py-4 font-black text-xs text-indigo-600 italic"><?= $s['reg_no'] ?></td>
                                    <td class="px-6 py-4 text-slate-900 font-bold uppercase text-xs"><?= $s['name'] ?></td>
                                    <td class="px-6 py-4 text-center">
                                        <span class="text-slate-900 font-black text-sm"><?= $s['score'] ?></span>
                                        <span class="text-slate-400 font-bold text-[10px]">/<?= $s['total_marks'] ?></span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <span class="inline-block border-2 border-slate-900 px-3 py-1 font-black text-[10px] uppercase <?= $s['percentage'] >= 40 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' ?>">
