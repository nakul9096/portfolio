<?php
// Force session check prior to header buffer streams
...
// Comprehensive Tracking Matrix Resolution (URL parameter, Form tracking body, or Session memory state)
$test_id = 0;
if (isset($_GET['id']) && (int)$_GET['id'] > 0) {
    $test_id = (int)$_GET['id'];
    $_SESSION['last_active_test_node'] = $test_id; // Set fallback recovery context
} elseif (isset($_POST['test_id']) && (int)$_POST['test_id'] > 0) {
...
// =========================================================================
// ADDED BACKEND PROCESSING ENGINE: Form Roster Matrix Update Execution
// =========================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // 1. Flush outdated configuration associations for this testing scope
        $delete_stmt = $pdo->prepare("DELETE FROM test_candidates WHERE test_id = ?");
        $delete_stmt->execute([$test_id]);

        // 2. Loop and re-bind new checked candidate mappings
...
<script>
// Automatically retain "Update Students" tab state focus context following database mutations
